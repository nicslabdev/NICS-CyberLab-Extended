...
@windows_lab_exchange_bp.post("/api/ssh/read-remote-json")
def read_remote_json():
    try:
        payload = request.get_json(force=True) or {}
        remote_path = str(payload.get("remote_path", "")).strip()
        max_bytes = int(payload.get("max_bytes", 5 * 1024 * 1024))

        data = _build_service().read_remote_json_file(
        ...
        current_app.logger.exception("windows_lab_exchange read_remote_json failed")
        return jsonify({"ok": False, "error": str(exc)}), 400
