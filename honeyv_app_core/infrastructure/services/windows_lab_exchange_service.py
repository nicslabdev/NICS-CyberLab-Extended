...
class WindowsLabExchangeService:
    ...
    def read_remote_json_file(self, remote_path: str, max_bytes: int = 5 * 1024 * 1024) -> dict[str, Any]:
        if not remote_path:
            raise ValueError("remote_path is required")
        ...
        try:
            ...
            if size > max_bytes:
                raise ValueError(f"Remote file is too large for JSON preview: {size} bytes. Max allowed is {max_bytes} bytes")

            remote_file = sftp.open(remote_path, "rb")
            raw_bytes = remote_file.read()

            text = self._decode_remote_text(raw_bytes)

            try:
                json_data = json.loads(text)
            except json.JSONDecodeError as exc:
                raise ValueError(f"Remote file is not valid JSON: {exc}") from exc
            ...
            self._write_manifest(
                {
                    ...
                    "remote_path": remote_path,
                    "size": size,
                    "modified_utc": result["modified_utc"],
                }
            )
    ...
    def _decode_remote_text(self, raw_bytes: bytes) -> str:
        ...
        raise ValueError(f"Unable to decode remote file as text: {last_error}")
